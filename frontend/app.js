...
const restartGenerationButton = $("#restartGeneration");
const retryGenerationButton = $("#retryGeneration");
...
function watchGenerationJob(jobId) {
  ...
  currentGenerationState = "running";
  ...
  pipelineSource.onmessage = event => {
    ...
    if (data.state === "cancelled") {
      pipelineSource.close();
      generateButton.disabled = false;
      currentGenerationState = "cancelled";
      progressStatus.textContent = data.status || "Processamento cancelado";
    }
  };
}
...
async function startGeneration() {
  ...
  // A failed probe is intentionally non-blocking: YouTube metadata can fail
  // transiently while the actual download is still available.
  try {
    const qualityResponse = await fetch("/api/source-quality", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ youtube_url: youtubeUrl.value.trim() }),
    });
    const source = await qualityResponse.json();
    const requestedHeight = Number(quality.value);
    const availableHeight = Number(source.max_height || 0);
    if (
      qualityResponse.ok
      && availableHeight > 0
      && availableHeight < requestedHeight
      && !confirm(
        `O YouTube oferece no mÃ¡ximo ${availableHeight}p para este vÃ­deo, `
        + `mas ${requestedHeight}p foi selecionado. Gerar mesmo assim?`
      )
    ) {
      return;
    }
  } catch (_) {
    // The job endpoint will still report a definitive download error.
  }
...
async function restartGeneration() {
  ...
  if (
    isRunning
    && !confirm(
      "O processamento atual serÃ¡ encerrado e reiniciado com as mesmas configuraÃ§Ãµes. Continuar?"
    )
  ) {
    return;
  }
  ...
  if (isRunning) {
    try {
      await fetch(
        `/api/jobs/${currentGenerationJobId}/cancel`,
        { method: "POST" }
      );
    } catch (_) {}
  ...
  currentGenerationState = "restarting";
  await startGeneration();
}

generateButton.addEventListener("click", startGeneration);
restartGenerationButton.addEventListener("click", restartGeneration);
retryGenerationButton.addEventListener("click", restartGeneration);
