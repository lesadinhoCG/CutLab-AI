    ...
    try:
        ...
        with JOBS_LOCK:
            ...
            cancel_requested = bool(
                JOBS.get(job_id, {}).get("cancel_requested")
            )
        ...
        if was_cancelled:
            update_job(
                job_id,
                state="cancelled",
                status="Processamento cancelado; pronto para reiniciar",
...
def cancel_job(job_id: str):
    with JOBS_LOCK:
        ...
        job["status"] = "Cancelando processamento"
...
@app.post("/api/source-quality")
def source_quality(request: SourceQualityRequest):
    """Check source resolution before an expensive transcription/render job."""
    ...
    if parsed.scheme not in {"https", "http"} or not is_youtube:
        raise HTTPException(
            status_code=400,
            detail="Informe uma URL HTTP(S) do YouTube para verificar a qualidade.",
        )
    try:
        return probe_youtube_quality(url)
    except Exception as exc:
        return {"max_height": 0, "mode": None, "warning": str(exc)}
...
def job_events(job_id: str):
    ...
    def stream():
        ...
        while True:
            ...
            if data["state"] in {"done", "error", "cancelled"}:
                break
