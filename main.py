...
# like â†’. Reconfigure stdout/stderr to UTF-8 so output works on all platforms.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="CutLab AI")
    ...
    print(f"Highlights:    {len(result['highlights'])} candidates â†’ kept top {len(result['shorts'])}")
    print("=" * 72)
    for i, s in enumerate(result["shorts"], 1):
        print(f"\n#{i}  score={s.get('score')}  {s.get('start_time'):.1f}s â†’ {s.get('end_time'):.1f}s")
